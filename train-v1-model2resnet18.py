...
def main():
    ...
    model_fcn8x = ResNetFCN(len(config.VOC_CLASSES)).to(config.DEVICE)


    # 定义优化器
    if config.OPTIMIZER == 'adam':
        optimizer = torch.optim.Adam(
            model_fcn8x.parameters(),
            lr=config.LR,
            betas=config.BETAS
        )
    ...
    # 损失函数
    ce_loss_fun = CrossEntropy2d(nclass=len(config.VOC_CLASSES), ignore_label=None)

    # 加载预训练模型
    if config.INIT_EPOCH > 0:
        load_checkpoint(config.LATEST_MODEL, model_fcn8x, optimizer, lr_scheduler, lr=config.LR, device=config.DEVICE)
    ...
    for epoch in range(config.INIT_EPOCH, config.NUM_EPOCHS):
        ...
        # save latest model
        save_model_name = './resnet18_latest_model.pth'
        save_checkpoint(model_fcn8x, optimizer, lr_scheduler, save_model_name)

        # validation
        if epoch % config.EVAL_EPOCH == 0:
            # do evaluation
            pixAcc, mIoU = validation(model_fcn8x, val_dataloader, len(config.VOC_CLASSES), config.DEVICE)
            print('Validation in Epoch {}, PA: {}, MIOU: {}'.format(epoch, pixAcc, mIoU))

            new_pred = (pixAcc + mIoU) / 2
            if new_pred > best_pred:
                best_pred = new_pred
                save_model_name = './resnet18_best_model.pth'
                save_checkpoint(model_fcn8x, optimizer, lr_scheduler, save_model_name)
