...
class VOCSegDataset(Dataset):
    ...
    def filter(self):
        ...
        :return:
        """
        images_filter, mask_filter = [], []
        for image, mask in zip(self.images, self.masks):
            size = Image.open(image).size   # (w, h)
            if size[0] >= config.CROP_SIZE[0] and size[1] >= config.CROP_SIZE[1]:
                images_filter.append(image)
                mask_filter.append(mask)
        return images_filter, mask_filter
    ...
    def __getitem__(self, index):
        ...
        if self.transform_data:
            image = self.transform_data(image=image)['image']
